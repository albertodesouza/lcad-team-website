...
│   ├── generate_html.py              # Script to update HTML with new data
...
2. **Parse the XML:**
   ```bash
   python scripts/parse_lattes.py 7573837292080522.xml
   ```
   This extracts publications and projects to `src/data/lattes_data.json`

3. **Regenerate HTML pages:**
   Use the AI assistant (Cursor) with the following prompt to regenerate the `publications.html` and `projects.html` pages:

### Prompt for Regenerating Lattes Pages

```
I have updated my Lattes XML file. Please:

1. Read the file src/data/lattes_data.json (generated by parse_lattes.py)
2. Update src/alberto/publications.html with:
   - All journal articles (sorted by year, most recent first)
   - All conference papers
   - All books and book chapters
   - Include DOI links when available
   - Keep the same visual style and structure

3. Update src/alberto/projects.html with:
   - All research projects (ongoing first, then completed)
   - Translate project names from Portuguese to English
   - Include funding agencies
   - Keep the same visual style and structure

4. Maintain the bilingual format: English title + original Portuguese title for projects
```
...
- **Research Projects:** Name, period, status, funding agencies, team members
